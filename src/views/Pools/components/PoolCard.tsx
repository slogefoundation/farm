...
import { Button, IconButton, useModal, AddIcon, Image } from '@wdefifinancebsc/uikit'
...
const PoolCard: React.FC<HarvestProps> = ({ pool }) => {
  ...
  const { onApprove } = useSalsaApprove(stakingTokenContract, sousId)
  const { onStake } = useSalsaStake(sousId, isBnbPool)
  const { onUnstake } = useSalsaUnstake(sousId)
  const { onReward } = useSalsaHarvest(sousId, isBnbPool)
  ...
  const isOldSalsa = stakingTokenName === QuoteToken.SLOGE
  ...
  return (
    <Card isActive={isCardActive} isFinished={isFinished && sousId !== 0}>
      ...
      <div style={{ padding: '24px' }}>
        ...
        <Label isFinished={isFinished && sousId !== 0} text={TranslateString(330, `${earnToken} earned`)} />
        <StyledCardActions>
